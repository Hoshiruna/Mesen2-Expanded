...
uint64_t Debugger::GetCpuCycleCount()
{
	switch(type) {
		...
		case CpuType::GenesisMain: return GetDebugger<type, GenesisDebugger>()->GetCpuCycleCount();
...
void Debugger::PauseOnNextFrame()
{
	...
	switch(_mainCpuType) {
		...
		case CpuType::GenesisMain: Step(CpuType::GenesisMain, 224, StepType::SpecificScanline, BreakSource::PpuStep); break;
	}
}
...
bool Debugger::IsDebugWindowOpened(CpuType cpuType)
{
	switch(cpuType) {
		...
		case CpuType::GenesisMain: return _settings->CheckDebuggerFlag(DebuggerFlags::GenesisDebuggerEnabled);
	}
...
void Debugger::GetCpuState(BaseState &dstState, CpuType cpuType)
{
	...
	switch(cpuType) {
		...
		case CpuType::GenesisMain: memcpy(&dstState, &srcState, sizeof(GenesisCpuState)); break;
	}
}
