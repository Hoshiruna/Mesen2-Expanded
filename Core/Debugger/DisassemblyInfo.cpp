...
void DisassemblyInfo::GetDisassembly(string &out, uint32_t memoryAddr, LabelManager* labelManager, EmuSettings* settings)
{
	switch(_cpuType) {
		...
		case CpuType::GenesisMain: GenesisDisUtils::GetDisassembly(*this, out, memoryAddr, labelManager, settings); break;
...
EffectiveAddressInfo DisassemblyInfo::GetEffectiveAddress(Debugger *debugger, void *cpuState, CpuType cpuType)
{
	switch(_cpuType) {
		...
		case CpuType::GenesisMain: return GenesisDisUtils::GetEffectiveAddress(*this, (GenesisConsole*)debugger->GetConsole(), *(GenesisCpuState*)cpuState);
	}
...
uint8_t DisassemblyInfo::GetOpSize(uint32_t opCode, uint8_t flags, CpuType type, uint32_t cpuAddress, MemoryType memType, MemoryDumper* memoryDumper)
{
	switch(type) {
		...
		case CpuType::GenesisMain: return GenesisDisUtils::GetOpSize(cpuAddress, memType, memoryDumper);
	}
...
bool DisassemblyInfo::IsJumpToSub()
{
	switch(_cpuType) {
		...
		case CpuType::GenesisMain: return GenesisDisUtils::IsJumpToSub((_byteCode[0] << 8) | _byteCode[1]);
	}
...
bool DisassemblyInfo::IsReturnInstruction()
{
	switch(_cpuType) {
		...
		case CpuType::GenesisMain: return GenesisDisUtils::IsReturnInstruction((_byteCode[0] << 8) | _byteCode[1]);
	}
...
bool DisassemblyInfo::IsUnconditionalJump()
{
	switch(_cpuType) {
		...
		case CpuType::GenesisMain: return GenesisDisUtils::IsUnconditionalJump((_byteCode[0] << 8) | _byteCode[1]);
	}
...
bool DisassemblyInfo::IsJump()
{
	...
	switch(_cpuType) {
		...
		case CpuType::GenesisMain: return GenesisDisUtils::IsConditionalJump((_byteCode[0] << 8) | _byteCode[1]);
	}
