...
auto M68000::disassembleInstructionLength(n32 pc) -> u32 {
  _pc = pc;
  n16 opcode = _readPC();
  disassembleTable[opcode]();

  u32 size = _pc - pc;
  // Branch with word displacement: _branch() peeks extension without consuming it.
  if((opcode & 0xf000) == 0x6000 && (opcode & 0x00ff) == 0x00) {
    size += 2;
  }
